...
function App() {
  ...
  return (
    <div className="App">
        ...
        <div className="mailBody" style={currentMails === 0 ? {backgroundImage: `url(${logoImg})`} : {}}>
         {refreshArray ? 
         <div>
           {currentMails > 0 ? (
             <div className={showBody ? 'gridOnmail' : 'notGrid'}>
              <div className={showBody ? 'theTable mbHide' : 'theTable'}> 
                 <div className="tableHeader">
                     <div>From</div>
                     <div style={showBody ? {display: 'none'} : {display: 'block'}}>To</div>
                     <div>Subject</div>
                     <div className="lastRow">Date <div className="filterBtn" onClick={filterDate}></div> </div>
                 </div>
                      ...
                      return (
                         <div>
                          <div className="eachMail" key={i}> 
                            <div onClick={()=> openMail(i, mail)}>{showBody ? mail.from.substring(0, 8)+'...' : mail.from} </div>
                            <div style={showBody ? {display: 'none'} : {display: 'block'}} onClick={()=> openMail(i, mail)}>{mail.recipient} </div>
                            <div onClick={()=> openMail(i, mail)}>{mail.subject.substring(0, 35)}...
                             {mail.attach ? <div className="haveAttach"></div> : '' }
                            </div>
                         
                            <div className="theDate">{getDate(mail.Date)}
                              <div className="removeMail" onClick={()=>removeEmail(i)}>Remove</div>
                            </div>
                          </div>
              ...
              {showBody ? <MailContent data={theEmail} showBody={setBody} ></MailContent> : ''} </div>
